...
**Görev:** Cycle 5 — Model Değerlendirme ve Optimizasyon  
**Değerlendirilen Model:** Hafta 4'te geliştirilen Sulama Optimizasyon Modeli (`irrigation_model.py`)

---

## 1. Giriş

Cycle 4'te geliştirilen `IrrigationOptimizer` sınıfı, toprak nemi, toprak sıcaklığı, hava nemi, hava sıcaklığı ve yağış miktarı verilerini kullanarak gereken sulama miktarını (litre) tahmin eden bir RandomForestRegressor modeli içermektedir. Bu raporda söz konusu modelin performansı ölçülmüş, farklı algoritmalarla karşılaştırılmış ve parametre optimizasyonu yapılmıştır.

---

## 2. Kullanılan Veri

Cycle 4 ile aynı synthetic veri yapısı kullanılmıştır:
...
Cycle 4 modelinin (RandomForest) yanı sıra 4 farklı algoritma daha test edilmiştir:

| Model | MAE | RMSE | R² | CV R² |
|---|---|---|---|---|
| **GradientBoosting** | **3.1387** | **6.3068** | **0.9234** | **0.9069** |
| RandomForest (Cycle 4) | 3.5351 | 6.9893 | 0.9059 | 0.8967 |
...
- **Cycle 4 RandomForest modeli** ikinci sıradadır. R² değeri 0.91 ile güçlü bir performans göstermektedir.
...
Cycle 4'teki varsayılan parametreler (`n_estimators=100, max_depth=10`) optimize edilmiştir:

| Parametreler | MAE | RMSE | R² |
|---|---|---|---|
| n=50, depth=5 | 4.2286 | 8.3558 | 0.8655 |
| n=100, depth=10 *(Cycle 4 varsayılanı)* | 3.5351 | 6.9893 | 0.9059 |
...
Bu yapılandırma ile Cycle 4 modelinin MAE değeri **3.5351'den 3.4865'e** düşürülmüştür (%1.4 iyileşme).
...
1. **En iyi algoritma GradientBoosting'dir.** Cycle 4 modelinin yerini alması önerilebilir.
