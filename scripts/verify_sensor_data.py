...
def display_raw_data(df: pd.DataFrame, limit: int = 10):
    ...
    display_cols = ["id", "sensor_id", "reading_timestamp", "moisture_percent", "soil_temperature_c", "depth_cm"]
...
def check_data_quality(df: pd.DataFrame):
    ...
    if len(invalid_moisture) > 0:
        print("   └─ ⚠ UYARI: Aralık dışı nem değerleri tespit edildi!")
    ...
    if len(invalid_temp) > 0:
        print("   └─ ⚠ UYARI: Aralık dışı sıcaklık değerleri tespit edildi!")
        print(f"      Hatalı değerler: {invalid_temp['soil_temperature_c'].tolist()}")
    else:
        print("   └─ ✓ TÜM sıcaklık değerleri geçerli aralıkta")
    ...
    if not has_nulls:
        print("   └─ ✓ Eksik veri yok - Veri bütünlüğü mükemmel!")
    ...
    if quality_score >= 95:
        print("   └─ ⭐ MÜKEMMELKalite")
    elif quality_score >= 80:
        print("   └─ ⭐⭐ İYİ Kalite")
    elif quality_score >= 60:
        print("   └─ ⭐⭐⭐ ORTA Kalite")
    else:
        print("   └─ ⭐⭐⭐⭐ DÜŞÜK Kalite - Uyarı!")
