...
def generate_sample_raw_data(num_records: int = 20) -> pd.DataFrame:
    ...
    moisture_values = [
        35.5, 42.3, 28.7, 65.2, 75.1,      # Geçerli veriler
        150.0, 200.5, -50.0,                # Hatalı (aralık dışı)
        np.nan, np.nan,                     # Eksik veriler
        45.0, 38.9, 52.1, 71.4, 88.3,      # Geçerli veriler
        105.0, -30.0,                       # Hatalı (aralık dışı)
        np.nan,                             # Eksik veri
        33.2, 67.8, 48.5                    # Geçerli veriler
    ]
    data['moisture_percent'] = moisture_values[:num_records]
    
    # Sıcaklık değerleri: geçerli, hatalı ve eksik
    temperature_values = [
        22.5, 21.8, 23.1, 24.2, 25.0,      # Geçerli veriler
        -99.0, 150.0, -85.5,                # Hatalı (aralık dışı)
        np.nan, np.nan,                     # Eksik veriler
        20.3, 22.7, 24.5, 26.1, 23.9,      # Geçerli veriler
        999.0, -50.0,                       # Hatalı (aralık dışı)
        np.nan,                             # Eksik veri
        21.5, 23.0, 25.5                    # Geçerli veriler
    ]
...
def clean_moisture_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    invalid_mask = (
        (df_cleaned['moisture_percent'] < MOISTURE_MIN_PERCENT) |
...
def clean_temperature_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    invalid_mask = (
        (df_cleaned['soil_temperature_c'] < TEMPERATURE_MIN_C) |
...
def clean_timestamp_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_cleaned['reading_timestamp'] = pd.to_datetime(
        df_cleaned['reading_timestamp'],
        errors='coerce'
    )
...
def print_data_summary(raw_df: pd.DataFrame, cleaned_df: pd.DataFrame, stats: dict):
    ...
    if len(cleaned_df) > 0:
        ...
        print(f"   ├─ Nem (%), min-max: {cleaned_df['moisture_percent'].min():.2f} - {cleaned_df['moisture_percent'].max():.2f}")
        print(f"   ├─ Sıcaklık (°C), ortalama: {cleaned_df['soil_temperature_c'].mean():.2f}")
        print(f"   ├─ Sıcaklık (°C), min-max: {cleaned_df['soil_temperature_c'].min():.2f} - {cleaned_df['soil_temperature_c'].max():.2f}")
