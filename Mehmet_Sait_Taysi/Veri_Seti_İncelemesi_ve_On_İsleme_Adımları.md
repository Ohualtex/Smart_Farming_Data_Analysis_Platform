...
o	Kısa süreli kesintiler (< 3 saat) için İleri/Geri Doldurma (Forward/Backward Fill) veya Doğrusal İnterpolasyon (Linear Interpolation) kullanılacaktır.
o	Uzun süreli kesintiler (> 3 saat) için, o günün tarihsel ortalaması veya hareketli ortalama (Moving Average) kullanılacaktır.
•	Kategorik Veriler (Örn: Toprak Tipi):
o	Eksik değerler veri setinin mod (en çok tekrar eden) değeri ile doldurulacak veya modelin öğrenmesi için "Bilinmiyor" olarak etiketlenecektir.
•	Görüntü Verileri:
o	Bozuk (corrupted) veya metadatasında eksiklik olan görüntü dosyaları analiz boru hattından (pipeline) tamamen çıkarılacaktır.
...
•	İstatistiksel Yöntemler: Tabular verilerde, verinin dağılımına göre Z-Skoru (Z-Score > 3) veya Çeyrekler Açıklığı (IQR) yöntemleri uygulanarak istatistiksel aykırılıklar tespit edilecektir.
•	Alan Bilgisine (Domain Knowledge) Dayalı Filtreleme: Mantıksal sınırların dışındaki değerler doğrudan filtrelenecektir.
o	Örnek: Toprak neminin %0'dan küçük veya %100'den büyük olması, pH değerinin 0-14 aralığı dışında olması gibi durumlarda bu veriler "Eksik Veri" (NaN) olarak işaretlenip, 2. adımdaki tamamlama yöntemlerine tabi tutulacaktır.
