...
class ReportService:
    ...
    def generate_pdf_report(data: dict) -> io.BytesIO:
        ...
        pdf.cell(0, 10, f"Analitik Raporu - Son {data.get('period_days', 30)} Gun", new_x="LMARGIN", new_y="NEXT", align="C")
        pdf.cell(0, 10, f"Olusturulma Tarihi: {datetime.now().strftime('%Y-%m-%d %H:%M')}", new_x="LMARGIN", new_y="NEXT", align="C")
        pdf.ln(10)
