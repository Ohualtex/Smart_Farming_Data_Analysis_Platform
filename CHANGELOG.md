...
## [Unreleased] — `REBUILD` + `fixroll` iyileştirme serisi (v1-v7)

`shiftFinal` bridge sprint 17 Mayıs'ta kapandı (`v0.9.0-pre-rebuild`
tag'i atıldı). 18 Mayıs'ta REBUILD sprint başladı; çiftçi-odaklı yeniden
yapılandırmanın 6 fazı + 81-il iddiası temizliği `main`'e merge edildi
(PR #24-25). Ardından **`fixroll` iyileştirme serisi (v1-v7)** ile frontend
cila, performans (N+1/index), hata-zarfı refactor, test/CI genişletme ve
production-risk bug fix turları yapıldı (PR #26-31 + v7). Bölüm açık kalır.

---

### `fixroll` iyileştirme serisi — v1-v7 (23-30 May 2026, ✅)

REBUILD merge'ü sonrası 7 kısa iyileştirme turu (`fixroll_v1` … `fixroll_v7`,
PR #25-31 + v7). Her tur birkaç bağımsız iş kalemi; mini-numaralandırma
commit mesajlarında (`fixroll_vN #M`).

#### Fixed (düzeltmeler)
- **fertilizer `/schedules` 500 → 422** (v7) — `planting_date` boş/geçersiz
  string [`fertilizer_service.py`](app/services/fertilizer_service.py)
  içindeki `datetime.strptime` ham `ValueError` fırlatıp **500** dönüyordu
  (Schemathesis fuzz bulgusu: `planting_date=""`). Fix:
  [`FertilizerScheduleRequest`](app/schemas/schemas.py) `field_validator` →
  artık **422** + 2 regresyon testi (boş + gün-ay-yıl formatı).
- **docker-compose `ENVIRONMENT` default** (v2) — `production` → `development`.
  `.env` yokken dev sentinel key'leriyle (`dev-api-key`/`dev-secret-key`)
  çelişip `_validate_production` fail-fast atıyordu; fresh clone'da
  `docker compose up` hiç ayağa kalkmıyordu.
- **Türkçe karakter normalize + ValidationError handler** (v3) — exception
  zarfında TR karakter bozulması.

#### Performance
- **N+1 query fix** (v3) — fields detail + alerts check tek batch sorguya.
- **Eksik FK index'leri** (v3) — `farms.user_id`, `fields.farm_id`,
  `fields.crop_id`.

#### Added / Changed (frontend)
- **Filiz maskot** — rol-aware tip havuzları + isim kişiselleştirme (v2);
  **critical-alert binding** (v6): farmer'a en yeni kritik uyarı balonu
  (%50 olasılık, sleepy mood'da sessiz).
- Harita auto-zoom `fitBounds` (v2), tarla detayda sensör ekle/sil UI (v2),
  empty-state cilası — dashed card + CTA (v5), inline form validation +
  `aria-invalid` (v5), mobile responsive cila (v5), backend hata-zarfı
  mesajını toast'a yansıtma (v5).

#### Refactor & altyapı
- **`HTTPException → SFDAPError` envelope** geçişi — middleware (v4) +
  router'lar (v5); tutarlı hata response zarfı, backward-compat.
- LOG_LEVEL env + slow-request warning (v3), Prometheus uptime exporter (v3),
  bandit false-positive config B101/B105/B311 (v3), npm dependency update (v5).
- Mimari + contributors doc REBUILD pivot senkronu (v2).

#### Test & CI
- **Backend 622 → 650**, **frontend 32 → 59** (vitest: map 29 + skeleton 14 +
  ui_helpers 16).
- Integration end-to-end + edge-case (auth/onboarding/RBAC) + N+1 path
  coverage testleri (v4); **CI Python 3.11/3.12 matrix** + lint pip cache (v4);
  axe-core ChromeDriver/Chrome senkron — CI yeşillendi (PR #31).

#### Doğrulama (v7 sonu)
- **pytest 650/650** · **coverage %95** · ruff check+format temiz · **vitest 59/59**.
