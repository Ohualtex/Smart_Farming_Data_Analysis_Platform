...
|:--|:--:|:--:|:--:|
| **Test sayısı** | 301 | — | ✅ |
| **Coverage** | %94.42 | %95+ | 🟡 (shiftFinal'da resmî tutuş) |
...
| **Endpoint sayısı** | 41 | — | ✅ |
| **ORM tablo sayısı** | 15 (yeni: aggregate) | — | ✅ |
| **Alembic migration zinciri** | 2 revision (initial + aggregate) | — | ✅ |
| **Frontend SPA satır sayısı** | ~3,700 (`frontend/index.html`) | bundling beklenir | 🟡 shiftFinal'da Vite |
...
| Coverage | %86 (zayıf: scheduler %50, mqtt %54) | %94.42 (40+ yeni test) | `cf35a63` + `15c154e` |
...
| # | Konu | Sahip (shiftFinal) | Etki |
|:--|:--|:--|:--|
| A1 | Coverage %95+ resmî tutuş — `metrics.py` (%81), `report_service.py` (%83) kapanmalı | Ayşe | Test guarantees |
| A2 | Sentry + Prometheus + structured logging | Mehmet | Prod observability |
| A3 | Vite bundling + a11y (ARIA, WCAG AA) + skeleton loaders | Ecenur | UX + akademik teslim |
| A4 | DB pool tuning + backup/restore script + cron | Emirhan | Prod resilience |
| A5 | `_clean_tr` 130-line refactor + magic numbers → constants | Miraç | Code quality polish |
| A6 | Pillow 14 hazırlığı (`getdata` → numpy) | Miraç | Future-proof (deprecation 2027) |

### Tier B — Orta değer, ileride

| # | Konu | Etki |
|:--|:--|:--|
| B1 | `app/config.py` %78 coverage — production fail-fast path test edilmeli | Test guarantees |
| B2 | Frontend (`index.html`) — 3,700 satır tek dosya, monolitik | Maintainability |
| B3 | `ANN001/ANN201` — public API'lerde return/arg type hints eksik (~580 ihlal, kademeli) | Type safety |
| B4 | `D101/D102` — public class/method docstring eksik (~260 ihlal) | API doc kalitesi |
| B5 | Edge case test paketi — 1MB JSON, unicode injection, race condition | Security guarantees |
| B6 | API responses Türkçe karakter normalizasyonu (RUF001/2/3, 1300+ ihlal — büyük çoğunluk false positive) | Doküman temizliği |
...
|:--|:--|:--|:--:|
| 13 May | Ayşe | Coverage push (`metrics`, `report_service`) | A1 |
...
| 17 May | Ayşe | Edge case testleri (1MB, injection, race) | A1+B5 |
| 17 May | Miraç | `_clean_tr` refactor + magic numbers | A5 |
| 18 May | Miraç | Pillow 14 prep + Tier B son cila | A6 |
...
```
Cycle başı  →  Cycle sonu (Cycle 8 kapanışı)
─────────────────────────────────────────────
Test         246    →  301     (+55 / +22%)
Coverage     %86    →  %94.42  (+8.4 pp)
LOC (app/)  ~3,300  →  ~4,100  (+~25%)
Endpoint     41     →  41      (≈, decorator eklendi)
